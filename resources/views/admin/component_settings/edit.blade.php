...
@section('content')
    ...
    <form action="{{ route('admin.component_settings.update',$component_setting->id) }}" method="POST">
        @csrf
            ...
            <div class="form-group ml-5">
                ...
                <div class="col-sm-9" id="key">
                    <input type="text" name="name" placeholder="name" id="name"
                           ...
                           value="{{old('name') ? old('name') : $component_setting->name}}">
                ...
                <div class="col-sm-9">
                    <div class="card card-custom">
                            ...
                            <div class="card-toolbar">
                                <ul class="nav nav-pills nav-pills-sm nav-dark-75 nav nav-test" role="tablist">
                                    <li class="nav-item nav-item">
                                        <span onclick="addKeyValue('type')" style="cursor: pointer"
                        ...
                        <div class="card-body">
                            <div class="key-value">
                                <div class="row">
                                    @foreach(json_decode($component_setting->settings) as $key=>$setting)
                                        <div class="col-md-4">
                                            <label for="key"
                                                   class="col-sm-4 col-form-label">{{__('message.Key')}} </label>
                                            <div class="col-sm-9" id="keyDiv_{{$key}}">
                                                <input type="text" name="key[]" placeholder="key" id="key"
                                                       class="form-control {{$errors->first('key') ? "is-invalid" : "" }} "
                                                value="{{old('key[]') ? old('key[]') : $key}}">
                                                <div class="invalid-feedback">
                                                    {{ $errors->first('key') }}
                                                </div>
                                            </div>
...
@push('scripts')
    ...
    <script>
        function addKeyValue(data) {
            var addid = 1;
            var addList = document.getElementById('keyDiv_'+data);
            ...
            var addid1 = 1;
            var addList1 = document.getElementById('valueDiv_'+data);
